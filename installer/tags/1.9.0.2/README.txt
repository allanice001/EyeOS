...
What's new in eyeos 1.9.0.2
	* Added Container widget method "addContent".
	* Added value synchonisation for Hidden widgets.
	* Added eyeBoard feature: Clicking an username starts a new internal message.
	* Improved PHP 5.3 compatibility.
	* Changed the Label widget's default "text" property value to an empty string instead of its id.
	* Changed the Select widget methods "addGroup" and "addOptions" to allow their second parameter to be empty (or null).
	* Changed the Toolbar widget methods "pressItem" and unpressItem" to allow strings as input.
	* Fixed Button widget method "addEvent".
	* Fixed eyeDock's week day support.
	* Fixed eyeSimpleDb library handling of empty select queries. Now they return an empty array instead of false.
	* Fixed eyex service method "messageBox" to add hiddens as friends.
	* Fixed eyex service method "runjs" handling of new line characters.
	* Fixed khtml browser engine support.
	* Fixed Tab widget style for some special cases.
	* Fixed Tab widget while removing tabs and resizing.
	* Fixed Textarea widget disabling.
	* Fixed Tree widget method "addSubList".
	* Fixed weekPlanner widget JavaScript warnings.
