...
//Files
define('UPDATE_PHP','update.php');
define('UPDATE_PACKAGE','eyeOS-1.0.1_1.0.2.eyepackage');
define('UPDATE_DIR','./');
define('UPDATE_CHANGES','eyeOSChanges.xml');
define('UPDATE_IMG_DIR','img/');

define('SETTINGS','settings.php');
define('EYEOS_HOST','soft.eyeos.org');
define('DELETE_FILES',FALSE);
define('EYEOS_UPDATE_VERSION','1.0.2');
define('eyeOS-1.0.1_1.0.2.eyepackage','57060b30f1ab039cdf6c92f96bce01e7');

define('UPDATE_MSG','Thanks for updating eyeOS!');

//Creating array with all critical files
	$files[] = UPDATE_PHP;
	$files[] = UPDATE_PACKAGE;
	$files[] = UPDATE_DIR;
	$files[] = UPDATE_CHANGES;
	
//Init functions	
	getSettings();//Gettigs settings.php
	getChanges();//Get the changes from xml
	
	updateConfigs();
//Check system		
	checkCurrentVersion();//Check if current version are just below one of the updater version
	checkFileExists($files);//Check if all update files inside $files array exists
	checkPermissions($files);//Check if all update files inside $files array are writeable and readable
	checkIntegrationFiles(array(UPDATE_PACKAGE));//Check if UPDATE_PACKAGE are nice (md5sum); the function support more than 1 file 
	checkPreInstall();//Check pre-install conditions, for example if php are in safe_mode
//update eyeos, first check the step.
	chooseStep();
...
function chooseStep(){
	if(isset($_GET['step']) && $_GET['step'] == 1){		
		...
		$listFiles = PclTarList(UPDATE_PACKAGE,'tgz');
		show_face1($listFiles);
	}
}
...
function checkCurrentVersion(){
	$updateVersion = intval(str_replace('.','',EYEOS_UPDATE_VERSION));//I don't know if (int) cast exsts in php4, for this reason use intval.
	$currentVersion = intval(str_replace('.','',EYEOS_VERSION));
	if($updateVersion > $currentVersion){
		return true;
	}elseif($updateVersion == $currentVersion){
		//TODO: put error hanlder system
		//At the moment...
		$errors[] = 'The current eyeOS installation already have this update.';	
		show_faceError1($errors);
		exit;	
	}elseif($updateVersion < $currentVersion){
		$errors[] = 'The current eyeOS installation is more update.';//TODO: fix english.	
		show_faceError1($errors);
		exit;	
	}
}
function checkRootPath(){
	if(file_exists('settings.php') && file_exists('index.php')){
		return true;
	}else{
		$error[] = 'The eyeOSupdater will be in root folder, where are index.php and settings.php.';
		show_faceError1($error);
		exit;
	}
}
...
/**Initialitation functions (no checks)
 *  Include settings.php //For get the eyeOS configuration.
 *  ...
 **/
...
//Get the changes from UPDATE_CHANGES file and make a global array with contents.
function getChanges(){
	global $changesArray;
	if(!file_exists(UPDATE_CHANGES)){
		return false;//TODO: put error hanlder
	}else{
		$fp = fopen(UPDATE_CHANGES,'r');
		$xml = fread($fp,filesize(UPDATE_CHANGES));
		$changesArray = lib_eyeXML_xml2array(array($xml));
		fclose($fp);
	}
}

/**Update functions
 *  delete files from xml
 *  ...
 **/
 function deleteFiles(){
	global $changesArray;
	if($changesArray['deleteFiles']){
		foreach($changesArray['deleteFiles'][0] as $file=>$value){
			$filePath = EYE_ROOT.'/'.$value[0];//EYE_ROOT is from settings.php and value[0] is the path from xml.
			if(file_exists($filePath)){
				unlink($filePath);	
			}else{
				//TODO: put the file don't exists in error hanlder system
			}			
		}
		unset($changesArray['deleteFiles']);
	}else{
		return true;//Nothing for delete
	}
}
function updateConfigs(){
	global $changesArray;
	//foreach direct in updateConfigs array
	if(isset($changesArray['updateConfigs'])){
		foreach ($changesArray['updateConfigs'][0] as $configName=>$configChanges){
			$path = $configChanges[0]['path'][0];//TOOD: put eye_root
			//Getting xml
			$fp = fopen($path,'r');
				$xml = fread($fp,filesize($path));
			fclose($fp);
			//Xml to array for merge the new config
			$arrayConfig = lib_eyeXML_xml2array(array($xml));
			
			//Now, start parse configChanges, addValue is the first :)
			//if have any addvalue in changeXml
			if(isset($configChanges[0]['addValue'][0])){
				//Adding new values in config array.
				foreach($configChanges[0]['addValue'][0] as $value =>$content){
					$arrayConfig['CONFIG'][0][$value][0] = $content[0];
				}
			}
			if(isset($configChanges[0]['updateValue'][0])){
				//Update defined values. 
				foreach($configChanges[0]['updateValue'][0] as $value =>$content){
					$arrayConfig['CONFIG'][0][$value][0] = $content[0];
				}
			}
			if(isset($configChanges[0]['delValue'][0])){
				//Delete values 
				foreach($configChanges[0]['delValue'][0] as $value =>$content){
					unset($arrayConfig['CONFIG'][0][$value]);
				}
			}
			//Convert array 2 xml for write this in file.
			$xml = lib_eyeXML_array2xml(array($arrayConfig));
			//And finaly, write the new config in his file.
			$fp = fopen($path,'w+');
				fwrite($fp,$xml);
			fclose($fp);			
		}
...
function updateIndex(){
	global $changesArray;
	if(!$changesArray['updateIndex'][0]){
		return true;//Do nothing because the new index not are defined in xml
	}
	$newIndex = base64_decode($changesArray['updateIndex'][0]);
	$fp = fopen('index.php','w');	
		fwrite($fp,$newIndex);
	fclose($fp);
	unset($changesArray['updateIndex']);
	//TODO: put any error here?	
}
function changeCurrentVersion(){
	$fp = fopen('settings.php','r');
	$settings = fread($fp,filesize('settings.php'));
	fclose($fp);
	//TODO: when settings parser lib are made, replace with it
	$settings = str_replace("define('EYEOS_VERSION','".EYEOS_VERSION."');",//search old version
							"define('EYEOS_VERSION','".EYEOS_UPDATE_VERSION."');",//replace with new version
							$settings//text settings vars
							);
...
function deleteUpdateFiles(){
	...
	unlink(UPDATE_PHP);
	unlink(UPDATE_CHANGES);
	unlink(UPDATE_IMG_DIR.'/default.jpg');
	unlink(UPDATE_IMG_DIR.'/installText.png');
	unlink(UPDATE_IMG_DIR.'/logo.png');
	unlink(UPDATE_IMG_DIR.'/window.png');
	rmdir(UPDATE_IMG_DIR);
}
function updateEyeOS(){
	//Delete files from $changesArray (xml)
	deleteFiles();
	//Extracting the eyepackage (add and update system files) (no configs);
	lib_eyeCompress_extractFile(array(UPDATE_PACKAGE));
	//Replace the current index for a new index stored in xml (base64 encoded);
	updateIndex();
	//Write in the settings.php the new version.
	changeCurrentVersion();
	//Delete instalation files, like eyepackage etc
	deleteUpdateFiles();
	//show a final face.
	show_face2(UPDATE_MSG);
}
		
function show_face1($updateFiles){	
globalImages();
global $wallpaper;
...
<head>
	<style type="text/css">
		body{
			background:url(data:image/jpeg;base64,'.$wallpaper.');
			font-family: verdana;
			font-size: 11px;
		}
		#windowContainer{
	...
	<title>eyeOS Updater</title>
</head>
<body id="bodyClass">
	<div id="windowContainer">
		<div id="logo"></div><div id="installText"></div>
		<div id="titulo">Note that if you edited any system default file after installation, it will be overwritten by the updater.</div>
		<div id="errorContent">';
		$counter = 1;
		foreach ($updateFiles as $error){
			echo "<div class='errorTextContainer'>";
				echo "<span class='numList'>$counter. </span><span class='textList'>".$error['filename']."</span>";
			echo "</div>";
			$counter++;
		}		
		echo '</div>
		<div id="buttonContent">
				<form action="update.php?step=1" method="post">
					<input type="submit" id="sendButton" value="Update eyeOS"/>
				</form>
		</div>
	</div>
</body>
...
<body id="bodyClass">
	<div id="windowContainer">
		<div id="logo"></div><div id="installText"></div>
		<div id="titulo">eyeOS Sucessfully updated from '.EYEOS_VERSION.' to '.EYEOS_UPDATE_VERSION.'</div>
		<div id="errorContent">';
			echo "<div class='errorTextContainer'>";
				echo "<span class='textList'>$msg</span>";
			echo "</div>";
		echo '</div>	
	</div>
</body>
...
<body id="bodyClass">
	<div id="windowContainer">
		<div id="logo"></div><div id="installText"></div>
		<div id="titulo">There were problems during update:</div>
		<div id="errorContent">';
		$counter = 1;
...
function lib_eyeCompress_extractFile($params){
	$file = $params[0];
	$dirToExtract = $params[1];
	$mode = $params[2];	//The mode parser will be do in pclTar function
    return PclTarExtract(UPDATE_PACKAGE,EYE_ROOT,'',$mode);
}
