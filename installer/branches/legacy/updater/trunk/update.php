...
define('UPDATE_PACKAGE','eyeOS-1.0_1.0.1.eyepackage');
define('UPDATE_DIR','./');
...
define('EYEOS_UPDATE_VERSION','1.0.1');
define('eyeOS-1.0_1.0.1.eyepackage','cb8aeacf3528b8001e4952383b28b13e');

define('UPDATE_MSG','Thanks for updating eyeOS!');
define('ERROR_OWNER','-1');
define('ERROR_MOD','-2');
...
//Creating array with all critical files
	...
	//Chekking if all file exists 	
    $fileReport = checkFileExists($files);
    if($fileReport != 1){
    	foreach ($fileReport as $file=>$value){
...
//Checking if eyeos was installed
	//Getting old eyeos version.
	define('OLD_EYEOS_VERSION',getEyeosVersion());
	$indexFile = SETTINGS;	
	if(!file_exists($indexFile)){
		$errors[0] = 'You need to extract me into the root of your eyeOS system.';
		show_faceError1($errors);
		exit(1);
	}else{
		if(OLD_EYEOS_VERSION == EYEOS_UPDATE_VERSION){
			$errors[0] = 'Your sistem was already updated to eyeOS '.EYEOS_UPDATE_VERSION;
			show_faceError1($errors);
			exit(1);
		}
	}
		   
    //Cheking if the critical files have a correct permissions
	$fileReport = reportPermissions($files);
	if($fileReport != 1){
		//TODO: make a point with can't change permissions because the file owner are diferent user
		foreach ($fileReport as $file=> $value){
	...
	//Cheking if package file are correct //the function is prepared for check more files
	$fileReport = checkIntegrationFiles(array(UPDATE_PACKAGE),0);
	if($fileReport != 1){
		//TODO: make a point with can't change permissions because the file owner are diferent user
		foreach ($fileReport as $file=> $value){
	...
	//Cheking prerequisites this function don't return any and it self call show_error if needed
	checkPreInstall();
	
	//If install files don't have permission errors, show forms or go to install
	if(isset($_GET['step']) && $_GET['step'] == 1){		
		updateEyeOS();//update eyeos	
	}else{
		$extractFiles = pclTarList(UPDATE_PACKAGE,'tgz');
		foreach($extractFiles as $file){
			$filesName[] .= $file['filename'];
		}
		show_face1($filesName);
	}
	...
	function updateEyeOS(){
		$eyeroot = getEYE_ROOT();
		$result = extractPackage($eyeroot);
		setAllowPublicDirs();
		if(DELETE_FILES == true)
		{
			unlink(UPDATE_DIR.'/'.UPDATE_IMG_DIR.'/default.jpg');
			unlink(UPDATE_DIR.'/'.UPDATE_IMG_DIR.'/installText.png');
			unlink(UPDATE_DIR.'/'.UPDATE_IMG_DIR.'/logo.png');
			unlink(UPDATE_DIR.'/'.UPDATE_IMG_DIR.'/window.png');
			rmdir(UPDATE_DIR.'/'.UPDATE_IMG_DIR);
	...
	function reportPermissions($files){
		$errorPermissions = checkCurrentPermissions($files);
		if($errorPermissions == true){
			return true;
		}else{			
			return changeCurrentPermissions($errorPermissions);
		}
	}

	function extractPackage($dirToInstall){		
			lib_eyeCompress_extractFile(array($dirToInstall));
	}
	...
	function checkCurrentPermissions($filesToCheck){	
		foreach($filesToCheck as $file){				
			$arrayPerms = checkPermission($file);
			foreach($arrayPerms as $permission =>$value){
				if($value == FALSE)
				{
	...
	function checkPermission($file){
		if (is_readable($file)) {	    	
	    	$filePermission['R'] = true;
		} else {	    	
	    	$filePermission['R'] = FALSE;
		}	
		if (is_writable($file)) {		
	    	$filePermission['W'] = true;
		} else {
	    	$filePermission['W'] = FALSE;
		}
		//is_executable don't work in windows.
		//TODO: check if WINNT si always same in all windows.
		if(PHP_OS != 'WINNT')
		{
			if (is_executable($file)) {
			$filePermission['X'] = true;
			} else {
	    		$filePermission['X'] = FALSE;
			}		
		}		
		return $filePermission;
	}
	
	//Yes i know, resolve domain is not a nice internet test, but need more?
	function checkInternetConnection(){
		
		if(gethostbyname(EYEOS_HOST) == EYEOS_HOST){
			if(!defined(EYEOS_CONN)){
				define('EYEOS_CONN',FALSE);
			}
			return false;
		}else{
			if(!defined(EYEOS_CONN)){
				define('EYEOS_CONN',true);
			}
			return true;
		}
	}
	
	function checkIntegrationFiles($filesToCheck,$mode){
		if($mode == 1){
		//TODO: checking the updated hash from eyeos server in xml.
		}else if($mode == 0){			
			foreach($filesToCheck as $file){
				if(is_dir($file)){
					continue;
				}else{
				$hashes[$file] = constant($file);	
				}				
			}
		}
		foreach($filesToCheck as $file){	
			...
			if(!is_dir($file)){
				if($hashes[$file] != $md5Sum){
					$arrayResult[$file] = FALSE;				
				}
			}
		}
	...
	function changeCurrentPermissions($currentPermission){		
		//TODO: make user comprobation for unix system and investigate how do it with win host. 
		foreach ($currentPermission as $file)
		{								
			if(!chmod($file,0777))
			{
				$errorPermissions[$file] = ERROR_MOD;
			}
		}
	...
	function setAllowPublicDirs(){
		$configFile = getEYE_ROOT();
		$configFile = $configFile.'/'.'system/conf/system.xml';
		
		$fp = fopen($configFile,'r+');
		$xml = fread($fp,filesize($configFile));
		fclose($fp);			
		$array = lib_eyeXML_xml2array(array($xml));
		
		//Adding new config param
		$array['CONFIG'][0]['ALLOW_PUBLIC_DIRS'][0] = 1;
		
		$xml = lib_eyeXML_array2xml(array($array));
	...
	function getEyeosVersion(){
		if(defined('EYEOS_VERSION')){
			return EYEOS_VERSION;
		}else{
			include_once(SETTINGS);
			return EYEOS_VERSION;	
		}
...
<body id="bodyClass">
	<div id="windowContainer">
		<div id="logo"></div><div id="installText"></div>
		<div id="titulo">eyeOS Sucessfully updated from '.OLD_EYEOS_VERSION.' to '.EYEOS_UPDATE_VERSION.'</div>
		<div id="errorContent">';
			echo "<div class='errorTextContainer'>";
...
function getArrayText($myArray,$loops) {
	...
	$enter = 0;
	if(!is_array($myArray)) {
		return htmlspecialchars($myArray);
	}
...
function lib_eyeCompress_extractFile($params){
	$dirToExtract = $params[0];
    /* Init. ArchiveExtractor Object */
	$archExtractor=new ArchiveExtractor();
	$hola = $archExtractor->extractArchive(UPDATE_PACKAGE,$dirToExtract);
}
...
class ArchiveExtractor {
  ...
  */
  function extractArchive($archFile, $extractPath=".") {
    ...
    if(eregi("tar|gz|tar.gz|tgz|eyepackage",$archFile)) {
  ...
  */
  function extractTarGzip($archFile, $extractPath=".") {
    /* include TAR library */
    //require_once 'libCompress/pcltar.func.eyecode';
...
//        if ($p_file_list[$i] == $v_header[filename])
  ...
  function PclTarHandleExtension($p_tarname)
  {
    ...
    if ((substr($p_tarname, -7) == ".tar.gz") || (substr($p_tarname, -4) == ".tgz") || (substr($p_tarname, -11) == ".eyepackage"))
    {
