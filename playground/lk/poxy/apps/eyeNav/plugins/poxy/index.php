...
$GLOBALS['_config']            = array
                    (
                        'url_var_name'             => 'page', // eyeOS
                        'flags_var_name'           => 'hl',
                        'get_form_name'            => '____pgfa',
                        'basic_auth_var_name'      => '____pbavn',
                        'max_file_size'            => -1,
                        'allow_hotlinking'         => 1, // eyeOS
                        'upon_hotlink'             => 1,
                        'compress_output'          => 1 // eyeOS
                    );
$GLOBALS['_flags']             = array
                    (
                        'include_form'    => 0, // eyeOS
                        'remove_scripts'  => 0, // eyeOS
                        'accept_cookies'  => 1,
                        'show_images'     => 1,
                        'show_referer'    => 1,
                        'rotate13'        => 0,
...
$GLOBALS['_version']           = '0.5b2';
$GLOBALS['_http_host']         = isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : (isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : 'localhost');
// $GLOBALS['_script_url']        = 'http' . ((isset($_ENV['HTTPS']) && $_ENV['HTTPS'] == 'on') || $_SERVER['SERVER_PORT'] == 443 ? 's' : '') . '://' . $GLOBALS['_http_host'] . ($_SERVER['SERVER_PORT'] != 80 && $_SERVER['SERVER_PORT'] != 443 ? ':' . $_SERVER['SERVER_PORT'] : '') . $_SERVER['PHP_SELF']; // eyeOS
// $GLOBALS['_script_base']       = substr($GLOBALS['_script_url'], 0, strrpos($GLOBALS['_script_url'], '/')+1); // eyeOS
$GLOBALS['_url']               = '';
...
if (!isset($GLOBALS['_proxify'][$GLOBALS['_content_type']]))
{
	...
	if (substr(implode('', $GLOBALS['_response_headers']['content-type']), 0, 5) == 'appli') { // eyeOS
...
else
{
    ...
    for ($i = 0, $count_i = count($matches[0]); $i < $count_i; ++$i)
    {
            ...
            switch ($tag)
            {
                ...
                case 'form':
                    if (isset($attrs['action']))
                    {
                        ...
                        {
							$attrs['action'] = $GLOBALS['_script_url']; // eyeOS
							$extra_html.= '<input type="hidden" name="msg" value="doOutput" />'; // eyeOS
							$extra_html.= '<input type="hidden" name="checknum" value="' . $checknum . '" />'; // eyeOS
                        }
        ...
        if ($rebuild)
        {
            ...
            {
                $delim = strpos($value, '"') && !strpos($value, "'") ? "'" : '"';
                $new_tag .= ' ' . $name . ($value !== false ? '=' . $delim . str_replace('&', '&amp;', $value) . $delim : ''); // eyeOS
            }

            $GLOBALS['_response_body'] = str_replace($matches[0][$i], $new_tag . '>' . $extra_html, $GLOBALS['_response_body']);
        }
    }
	$GLOBALS['_response_body'] = str_replace('<body>', '<body onload=\'window.parent.sendMsg(' . $checknum . ', "SetAddress", "&lt;address&gt;' . base64_encode($GLOBALS['_url']) . '&lt;/address&gt;");\'>', $GLOBALS['_response_body']); // eyeOS
