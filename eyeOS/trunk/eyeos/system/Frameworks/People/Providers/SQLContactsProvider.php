...
class SQLContactsProvider {
	...
	public function searchContacts($text) {
		try {
			...
			$sqlQuery = 'SELECT id FROM eyeosuser WHERE name LIKE ? AND id != \'eyeID_EyeosUser_register\'';
			...
			$sqlQuery = substr($sqlQuery, 0, -15) . ' LIMIT 20';
			
			$stmt = $dbh->prepare($sqlQuery);
			for($i = 0; $i < count($text); $i++) {
				$string = '%' . $text[$i] . '%';
				$stmt->bindParam($i + 1, $string , PDO::PARAM_STR);
			}
