 ...
 * @subpackage Meta
 */
class PDOMetaProvider implements ISearchableMetaDataProvider {
	const PARAM_CONNECTIONSTRING = 'connectionString';
	...
	private $connectionKey = null;


	/**
	 * Constructs a new instance of PDOMetaProvider using the given $metaDataClass.
	 *
	 * @param string $metaDataClass
	 * @param array $params (PARAM_CONNECTIONSTRING, PARAM_TABLENAME, PARAM_USERNAME, PARAM_PASSWORD)
	 */
	...
	public function setParams($params) {
		if (!isset($params[self::PARAM_CONNECTIONSTRING]) || !is_string($params[self::PARAM_CONNECTIONSTRING])) {
			throw new EyeMissingArgumentException('Missing or invalid connection string argument: $params[' . self::PARAM_CONNECTIONSTRING . '].');
		}
	...
	public function setConnectionKey($params) {
		if (isset($params[self::PARAM_USERNAME])) {
			if (isset($$params[self::PARAM_PASSWORD])) {
				$connectionKey = $params[self::PARAM_USERNAME] . ':' . $params[self::PARAM_PASSWORD] . '@' . $params[self::PARAM_CONNECTIONSTRING];
			} else {
				$connectionKey = $params[self::PARAM_USERNAME] . '@' . $params[self::PARAM_CONNECTIONSTRING];
			}
		} else {
			$connectionKey = $params[self::PARAM_CONNECTIONSTRING];
		}
	...
	private function destroyConnection() {
		if (self::$Connections[$this->connectionKey] instanceof PDO) {
	...
	protected function getConnection() {
		if (! isset(self::$Connections[$this->connectionKey])) {
			$dbHandler = null;
			try {
				if (isset($this->params[self::PARAM_USERNAME])) {
					if (isset($this->params[self::PARAM_PASSWORD])) {
						$dbHandler = new PDO($this->params[self::PARAM_CONNECTIONSTRING], $this->params[self::PARAM_USERNAME], $this->params[self::PARAM_PASSWORD]);
					} else {
						$dbHandler = new PDO($this->params[self::PARAM_CONNECTIONSTRING], $this->params[self::PARAM_USERNAME]);
					}
				} else {
					$dbHandler = new PDO($this->params[self::PARAM_CONNECTIONSTRING]);
				}

				$dbHandler->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				$dbHandler->exec('CREATE TABLE IF NOT EXISTS ' . $this->params[self::PARAM_TABLENAME] . ' ('
				...
				self::$Connections[$this->connectionKey] = $dbHandler;
	...
	public function retrieveMeta($id, $params = null) {
		...
		try {
			...
			$stmt->execute(array(self::METADATA_FIELD_ID => $id));
	...
	private function searchSingleMeta($className, $key, $value) {
		...
		$stmt = $dbHandler->prepare($sqlQuery);
		$stmt->execute(array(self::METADATA_FIELD_CLASS => $className,
							 self::METADATA_FIELD_KEY => $key,
							 self::METADATA_FIELD_DATA => serialize($value)));
	...
	private function searchMultipleMetas($className, $metaDatas) {
		...
		foreach($metaDatas as $key => $value) {
			...
			$stmt->bindValue(':' . $key . '_value', serialize($value), PDO::PARAM_STR);
		}
		$stmt->execute();
