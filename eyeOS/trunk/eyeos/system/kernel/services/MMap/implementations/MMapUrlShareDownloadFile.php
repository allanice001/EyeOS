...
class MMapUrlShareDownloadFile extends Kernel implements IMMap {
	...
	public function processRequest(MMapRequest $request, MMapResponse $response) {
		...
		if($urlShare){
			if($urlShare->getEnabled()){
			    ...
			    self::renderNotEnabledFile($response,$urlShare->getName());
			}
	...
	private function downloadFile($filePath,$request,$response){
		...
		$path = $myFile->getRealFile()->getPath();
		$realPath = print_r(EYE_ROOT . '/' . $path,1);
		$codiFitxer = file_get_contents($realPath);
		...
		$response->getHeaders()->append('Content-Disposition: attachment; filename="' . urlencode($filename) . '"');
		$response->appendToBody($codiFitxer);
	}
