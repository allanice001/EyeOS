	...
	members: {
		...
		_updateBodyDescription: function () {
			...
			this.addListener('updateImage', function(e) {
				...
				if (oldSource != '') {
					var newSource = 'index.php?checknum=' + this.getChecknum() + '&message=getImg&params=' + e.getData();
			...
			cancelLabel.addListener('click', function (e) {
				this.setImage(null);
				this.fireDataEvent('updateImage', null);
			}, this);
		...
		_deleteAllTags: function () {
			var tagItems = this._secondColumn.getChildren();
			var size = tagItems.length;
		...
		_createSearchBox: function (){
			...
			searchClearIcon.addListener('click', function (e) {
				...
				this._searchTextField.setValue(tr('Search in your Contacts'));
		...
		_filterContacts: function (input) {
			...
			for (var i = 0; i < childrenList.length; ++i){
				if (childrenList[i].getName().indexOf(input) != -1){
