...
//					this.getView().showBrowse();
                        ...
                        if(selection) {
							...
							for(var i in selection) {
								...
								filesToMove.push({
										...
										getCreated: function() {
											if(!this.mInfo.meta.creationTime) {
												var foo = new Date;
												var unixtime_ms = foo.getTime();
												var unixtime = parseInt(unixtime_ms / 1000);
												return unixtime;
											}
											return this.mInfo.meta.creationTime;
										},

										getModified: function() {
											if(!this.mInfo.meta.modificationTime) {
												var foo = new Date;
												var unixtime_ms = foo.getTime();
												var unixtime = parseInt(unixtime_ms / 1000);
												return unixtime;
											}
											return this.mInfo.meta.modificationTime;
										},
		...
		openFile: function () {
			...
			var filesForSpreadsheets = new Array();
			var foldersToOpen = new Array();

			var extensionsForViewer = ['MP3','JPG','JPEG','FLV','HTM','HTML','M4A','PNG','GIF'];
			var extensionsForDocuments = ['EDOC'];
			var extensionsForSpreadsheets = ['ODS'];
			...
			for (var i = 0; i < filesToOpen.length; ++i) {
				...
				if (type == 'folder') {
					...
					if (extensionsForSpreadsheets.indexOf(extension) != -1) {
						filesForSpreadsheets.push(filesToOpen[i].getFile().getAbsolutePath());
					}
			...
			if (filesForSpreadsheets.length >= 1) {
				eyeos.execute('spreadsheets', this.getApplication().getChecknum(), filesForSpreadsheets);
			}

			if (filesForFemto.length >= 1) {
				eyeos.execute('femto', this.getApplication().getChecknum(), filesForFemto);
			}
		...
		renameFile: function (rename, object) {
		...
		toolBarForward: function () {
			if (this.getModel().getHistoryIndex() <= this.getModel().getHistory().length) {
