...
//					}
				...
				item.addListener('drop', function (e) {
					var droppedFile = e.getRelatedTarget().getFile();
					var droppedPath;
		...
		showBrowse: function () {
			...
			var files = this.reorder(this.getViewManager().getModel().getCurrentFiles());
			for (var i = 0; i < files.length; ++i) {
                                if(files[i].getName().indexOf(filter) >= 0) {
                                    var item = new eyeos.files.IconViewItem(this, files[i]);
                                    this._addItem(item);
                                }
			}
		...
		MENU: [
			{
				...
				label: tr('New spreadsheet'),
				image: 'index.php?extern=images/16x16/mimetypes/application-vnd.ms-excel.png',
				id: 'newFile(\'xls\')'
			}, {
				label: tr('New link'),
