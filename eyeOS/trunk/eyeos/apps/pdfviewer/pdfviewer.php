...
abstract class PdfViewerApplication extends EyeosApplicationExecutable {
    public static function convertFile($params) {
        ...
        if(!file_exists($conversionCache)) {
            ...
            $cmd = 'convert -verbose -quality 80 -density 300 '.$fileNameOriginal.' '.$fileNameDestinationRealSize;
		...
		if(!file_exists($conversionCache.$hash.'-0.jpg')) {
			...
			copy($conversionCache.$hash.'-thubnail.jpg', $conversionCache.$hash.'-thubnail-0.jpg');
		}
        $size = getimagesize($conversionCache.$hash.'-0.jpg');
        return array(((count($files)-2)/2), $hash, $size[0], $size[1]);
    }
    ...
    public static function getFile($params) {
        ...
        if(!$thubnail) {
            ...
            readfile($fileNameDestination.'/.office/'.$hash.'/'.$hash.'-thubnail-'.$num.'.jpg');
        }
        exit;
    }
}
