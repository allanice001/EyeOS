...
<manifest:manifest xmlns:manifest="urn:oasis:names:tc:opendocument:xmlns:manifest:1.0">
 ...
 <manifest:file-entry manifest:media-type="text/xml" manifest:full-path="settings.xml"/>
	...
	function addFormula($sheet,$row,$cell,$value) {
		$formula = "oooc:=[.".substr($value, 1); //Skip the initial '='
		$formula = str_replace('*', ']*[.', $formula);
		$formula = str_replace('+', ']+[.', $formula);
		$formula = str_replace('/', ']/[.', $formula);
		$formula = str_replace('-', ']-[.', $formula);
		$formula .= "]";
		$this->sheets[$sheet]['rows'][$row][$cell]['attrs'] = array('TABLE:FORMULA'=>$formula, 'OFFICE:VALUE-TYPE'=>'float','OFFICE:VALUE'=>$value);
		$this->sheets[$sheet]['rows'][$row][$cell]['value'] = $value;
	}
	
	function addGraphic($title) {
		
	}

	function editFormula($sheet,$row,$cell,$value) {
		$formula = "oooc:=[.".substr($value, 1); //Skip the initial '='
		$formula = str_replace('*', ']*[.', $formula);
		$formula = str_replace('+', ']+[.', $formula);
		$formula = str_replace('/', ']/[.', $formula);
		$formula = str_replace('-', ']-[.', $formula);
		$formula .= "]";
		$this->sheets[$sheet]['rows'][$row][$cell]['attrs']['TABLE:FORMULA'] = $formula;
		$this->sheets[$sheet]['rows'][$row][$cell]['attrs']['OFFICE:VALUE'] = $value;
		$this->sheets[$sheet]['rows'][$row][$cell]['value'] = $value;
	}
}
...
function saveOds($obj,$file) {
	...
	shell_exec('cd '.$tmp.'/'.$uid.';zip -r '.escapeshellarg(basename($file)).' ./');
	copy($tmp . '/' . $uid . '/'.basename($file),$file);
