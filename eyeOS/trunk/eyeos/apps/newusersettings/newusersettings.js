...
//		},
		...
		_addListeners: function () {
			...
			this._buttonAddFile.addListener('click', function (e) {
				...
				this._fileChooser.showOpenDialog(this.getWindow(), function(choice, path) {
					if (choice == eyeos.dialogs.FileChooser.APPROVE_OPTION) {
						eyeos.callMessage(this.getChecknum(), 'copy', ['home://~' + eyeos.getCurrentUserName() + '/Desktop/Images/', path], function (results) {
							eyeos.callMessage(this.getChecknum(), '__FileSystem_browsePath', ['home://~'+eyeos.getCurrentUserName()+'/Desktop/Images/', null, null], function (results) {
								this._populateWallpapers(results);
							}, this);
						}, this);
					}
			...
			this._buttonUploadFile.addListener('click', function (e) {
				this._selectDesktopUserImages();
				eyeos.execute('upload', this.getChecknum(), ['home://~' + eyeos.getCurrentUserName() + '/Desktop/Images', {allowChangePath: 'false'}]);
			}, this);
			...
			dbus.addListener('eyeos_file_uploadComplete', function (e) {
				var splitted = e.getData().absolutepath.split('home://~'+eyeos.getCurrentUserName()+'/Desktop/Images/');
				if (splitted[1] != undefined) {
					var toCheck = splitted[1].split('/');
					if (toCheck.length == 1) {
						this._populateWallpaper(e.getData(), 'home://~'+eyeos.getCurrentUserName()+'/Desktop/Images');
					}
				}
			}, this);
...
//					mailBox.add(this._mailBoxAddMail);
				...
				case 2: {
					...
					var itemsForList =
						[{
							...
							model: {
								id: 'blackwhite',
								path: 'sys:///extern/images/wallpapers/blackandwhite'
							}
						}, {
							name: 'Classic',
							model: {
								id: 'classic',
								path: 'sys:///extern/images/wallpapers/classic'
							}
						}, {
							name: 'Nature',
							model: {
							...
							model: {
								id: 'user',
								path: 'home://~' + eyeos.getCurrentUserName() + '/Desktop/Images/'
							}
						}];
		...
		_populateWidgets: function () {
			...
			document.widgets.forEach(function(obj) {
				var item = new qx.ui.form.CheckBox(obj.title);
				...
				item.addListener('changeValue', function(e) {
					...
					if (e.getTarget().getValue()) {
						e.getTarget().obj.widget = e.getTarget().obj.create(e.getTarget().obj.checknum, e.getTarget().obj.value);
		...
		_populateWallpaper: function(result, path) {
			...
			var pathToCheck = 'home://~' + eyeos.getCurrentUserName() + '/Desktop/Images';
			var url;
			var img;
			if (path != pathToCheck) {
				url = result.absolutepath.substr(32, result.absolutepath.length);
				img = 'index.php?extern=images/wallpapers/' + url;
			} else {
				img = 'index.php?checknum=' + this.getChecknum() + '&message=__FileSystem_readFile&params[path]=' + result.absolutepath;
			}
