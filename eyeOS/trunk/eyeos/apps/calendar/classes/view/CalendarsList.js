...
qx.Class.define('eyeos.calendar.view.CalendarsList', {
	...
	members: {
		...
		__init: function() {
			...
			//
			this._buttonsGroupBox = new qx.ui.groupbox.GroupBox().set({
				contentPaddingLeft: 11,
				contentPaddingRight: 11,
				contentPaddingTop: 2,
			...
			this._buttonsGroupBox.getLayout().setColumnFlex(1, 1);
...
qx.Class.define('eyeos.calendar.view.MyCalendarsList', {
	...
	construct: function () {
		arguments.callee.base.call(this, 'My Calendars');
		
		// Add "New" button on bottom-right
		var newCalendarButton = new qx.ui.form.Button(tr('New')).set({
			decorator: null,
			cursor: 'pointer'
		});
		this._buttonsGroupBox.add(newCalendarButton, {row: 0, column: 2});
