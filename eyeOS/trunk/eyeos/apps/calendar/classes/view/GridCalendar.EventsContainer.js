...
qx.Class.define('eyeos.calendar.view.GridCalendar.EventsContainer', {
	...
	construct: function (el, gridCalendar, mode) {
		...
		if (mode == this.self(arguments).MODE_STACK) {
			...
			this.addListener('click', this.__onMouseClick, this);
		}
		// PLANNING MODE
		else {
	...
	members: {
		...
		__onMouseClick: function(e) {
			var timeEnd = new Date(this.getDate());
			timeEnd.setDate(timeEnd.getDate() + 1);
			timeEnd.setMilliseconds(-1);
			
			// Initialize a new model object
			var eventModel = this.__controller.createNewEvent().set({
			...
			eventView.addListenerOnce('appear', function(e) {
				// Display popup
				var popup = new eyeos.calendar.view.EventPopup(eventView);
				popup.placeToWidget(eventView);
		        popup.show();
			}, this);
		},
		...
		__onMouseDown: function(e) {
			...
			var displayedPopup = this.__controller.getProcVar('eyeos.calendar.view.EventPopup.instance');
			if (displayedPopup) {
				try {
		...
		__stopDrawingEvent: function() {
			if (this.__eventDrawingContext != null) {
				this.autoLayoutEvents();
				
				// Display popup
				var popup = new eyeos.calendar.view.EventPopup(this.__eventDrawingContext.eventView);
				popup.placeToWidget(this.__eventDrawingContext.eventView);
		        popup.show();
		...
		displayEvents: function(events) {
			...
			for(var i = 0; i < events.length; i++) {
				...
				if (! event.getTimeStart().isSameDay(this.getDate()) || event.isAllDay()) {
					//eyeos.consoleLog('Event ' + event.getId() + ' "' + event.getSubject() + '" is out of range, skipping.');
					continue;
				} else {
