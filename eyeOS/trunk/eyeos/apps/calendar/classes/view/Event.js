...
qx.Class.define('eyeos.calendar.view.Event', {
	...
	construct: function (eventModel, controller) {
		...
		var deleteItem = new qx.ui.menu.Button('Delete', 'index.php?extern=images/16x16/actions/edit-delete.png');
		contextMenu.add(deleteItem);
		deleteItem.addListener('execute', function(e) {
			this.getController().deleteEvent(this.__eventModel);
		}, this);
	...
	members: {
		...
		_onClickDetailled: function(e) {
			if (this.__simpleClick && e.getButton() == 'left') {
				// Display popup
				var popup = new eyeos.calendar.view.EventPopup(this);
		...
		_onUpdateTime: function(e) {
			if (this.getMode() == this.self(arguments).MODE_DETAILLED) {
				...
				this.getChildControl('header-label').setValue(label);
