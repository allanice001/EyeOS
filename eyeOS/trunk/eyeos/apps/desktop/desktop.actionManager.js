...
qx.Class.define('desktop.actionManager', {
    ...
    members: {
        uploadFile: function() {
			eyeos.execute('upload', this.getChecknum());
		},

		openFile: function() {
			var selection = desktop.SelectionManager.getInstance().getSelection();
			for(var i in selection) {
				eyeos.openFile(selection[i].getUserData('path'), this.getChecknum());
			}
		},
		...
		editFile: function() {
			...
			var rename = function(e) {
				...
				if(!element.getUserData('fired')) {
					element.setUserData('fired', true);
					element.getUserData('parent').getChildControl('label').show();
					element.destroy();
					element.getUserData('parent').setLabel(newname);
					element.getUserData('parent').setUserData('path', 'home://~'+eyeos.getCurrentUserName()+'/Desktop/'+newname);
					if(newname == "" || !newname || oldname == 'home://~'+eyeos.getCurrentUserName()+'/Desktop/'+newname) {
						return;
					}
