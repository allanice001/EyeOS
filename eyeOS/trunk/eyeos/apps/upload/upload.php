    ...
    public static function getHtmlCode($params) {
        ...
        echo '
            <html>
				...
				<script language="JavaScript"> ' . $buffer . '</script>
                <script>
                function startProgress() {
                    ...
                    var checknum = "'.$currentProc->getChecknum().'";
                ...
                function uploadDragAndDrop() {
                    ...
                    xhr.open("POST", "index.php?message=submitFile&checknum=' . $currentProc->getChecknum() . '&params=' . urlencode($path) . '", true);
				...
				<img style="position:relative;top:5px;" src="eyeos/extern/images/22x22/actions/arrow-up-double.png" />&nbsp;'. $langTitle .'
                </div>
                <div id="progressbar" style="margin-top:60px;display:none;">
					<div style="margin-bottom:6px;font-family:Helvetica, Arial, Verdana, Sans, FreeSans"><center>'. $langText .'...</center></div>
                    <div id="wrapper" style="width:410px;border:1px solid black;height:18px;margin-left:10px">
                        <div id="internal" style="width:0px;background-image:url(\'eyeos/extern/images/bg_progress.png\');background-repeat:repeat-x;height:18px;"></div>
                ...
                echo 'margin-top:20px;margin-left:10px" form action="index.php?message=submitFile&checknum=' . $currentProc->getChecknum() . '&params=' . urlencode($path) . '" method="post" enctype="multipart/form-data">
                    <input type="hidden" name="UPLOAD_IDENTIFIER" id="progress_key"  value="'.$id.'"/>
                    <div id="filedatas" style="overflow:auto">
                    <input type="file" name="Filedata[0]" size="22" id="Filedata[0]" multiple="true"/>
                    </div>
                    ...
                    <div>
                    <input type="submit" onclick="startProgress()" value="'. $langUpload .'" style="position:absolute;bottom:5px;right:9px;"/>
                    </div>
                </form>
            </html>
        ';
